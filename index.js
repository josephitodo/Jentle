...
async function startBot() {
  ...
  if (!sock.authState.creds.registered) {
    setTimeout(async () => {
      try {
        const code = await sock.requestPairingCode("2348106184386")
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    try {
      ...
      if (isOwner(sender)) {
        const moderation = await handleModeration(sock, jid, lower)
        if (moderation) return

        const admin = await handleAdmin(sock, jid, msg, lower)
        if (admin) return

        const group = await handleGroup(sock, jid, text)
        if (group) return
      }
